...
public class ServerController {
    ...
    //Порт как константа (Тоже мб в конфиг закину потом)
    private int port;

    //Дочерний процесс, в котором выполняется сервер
    //Нужно для того, чтобы пока сервер фунциклирует(ха-ха) в бесконечном цикле
    // и ждет новые подключения, мы могли жмакнуть на кнопку и отключить его
    private ServerProcessingThread serverProcessingThread;

    //Улавливатель исключений в дочернем потоке (Выводит на консоль исключения)
    ...
    private static Properties getPropertiesFromConfig() throws IOException {
        ...
        String propFileName = "ServerApp/TransferLayer/src/main/resources/config.properties";
    ...
    public void onRunServerButtonClick(ActionEvent actionEvent) {
        ...
        try {
            ...
            //Изменение цвета label'ов и прочей хуйни
